...
for num_file=start_num:NumFile
    ...
    for run=1:15 % run time
        tic;
        solution=adjust(distance,timewindow,servicetime,quality,depart,worktime,syn_points,num_caregiver,num_patient,num_syn,lunch_points,Maxgen,demand); %generate the initial solution
        bestroute=zeros(Maxgen,num_patient+2*num_caregiver+num_syn+1);
        bestoutputs(1)=object_ls(solution,distance,depart,lunch_points);
        bestroute(1,:)=solution;
        T=bestoutputs(1); %the initial temperature
        Te=0.1; %the terminal temperature
        bnz=0.99; %Boltzmann's constant
        j=1;
        while T>=Te
            current_route=bestroute(j,:);
            output_current=object_ls(current_route,distance,depart,lunch_points);
            for n=1:50  %max iterations for generating new solution at a temperature           
                [pop_search,output]=local_search(current_route,distance,timewindow,servicetime,quality,depart,worktime,syn_points,num_caregiver,num_patient,num_syn,lunch_points);  % local search             
                if output==output_current
                    pop_search=localmoves(pop_search,distance,timewindow,servicetime,quality,depart,syn_points,worktime,num_caregiver,num_patient,num_syn,lunch_points); % local moves
                    output=object_ls(pop_search,distance,depart,lunch_points);
                end
                delta=output-output_current;
                p=exp(-delta/T);
                if rand<p
                    current_route=pop_search;
                    output_current=output;
                end
                if output<bestoutputs(j)
                    bestoutputs(j+1)=output;
                    bestroute(j+1,:)=pop_search;
                    count(j)=1;
                else
            ...
            T=T*bnz;
            if max(not_change)>=10*num_patient  %max iterations wihout improving the best solution
                break
            end
        end
